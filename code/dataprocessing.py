...
def read_review(datatype="TRAIN", path=Path("..")):
    ...
    if datatype == "TRAIN":
        file = path / datatype / "Train_laptop_reviews.csv"
    elif datatype == "TEST":
        file = path / datatype / "Test_makeup_reviews.csv"
...
def read_label(datatype="TRAIN", path=Path("..")):
    ...
    file = path / datatype / "Train_laptop_labels.csv"
    df = pd.read_csv(file, sep=",")
    return df
